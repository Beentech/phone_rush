...
glbloader.load(
  ...
  function (gltf){
    ...
    const runningAction = gltf.animations.find((animation) => animation.name === "runnning");
    if (runningAction){
...
// 障害物の描画
for (let g = 1; g < 12; g++) {
  geometry = new ConeGeometry(1, 3, 32);
...
function collision() {
  ...
  enemy_list = enemy_list.filter((enemy) => {
    ...
    const isCollited = playerBoundingBox.intersectsBox(enemyBoundingBox);
    if (isCollited){
  ...
  phone_list = phone_list.filter((phone) => {
    ...
    const isCollited = playerBoundingBox.intersectsBox(phoneBoundingBox);
    if (isCollited){
      scene.remove(phone);
      return false;
    }
